...
UserDictManager::UserDictManager(Deployer* deployer)
    : deployer_(deployer),
      user_db_component_(UserDbComponent::Require("userdb") {
  if (deployer) {
...
bool UserDictManager::Backup(const std::string& dict_name) {
  unique_ptr<UserDb> db(user_db_component_->Create(dict_name));
  if (!db->OpenReadOnly())
    return false;
...
bool UserDictManager::Restore(const std::string& snapshot_file) {
  unique_ptr<UserDb> temp(user_db_component_->Create(".temp"));
  if (temp->Exists())
    temp->Remove();
  if (!temp->Open())
    return false;
  ...
  if (!temp->Restore(snapshot_file))
    return false;
  if (!UserDbUtil::IsUserDb(temp.get()))
    return false;
  std::string db_name = UserDbUtil::GetDbName(temp.get());
  if (db_name.empty())
    return false;
  unique_ptr<UserDb> dest(user_db_component_->Create("db_name"));
  if (!dest->Open())
    return false;
  ...
  LOG(INFO) << "merging '" << snapshot_file
            << "' from " << UserDbUtil::GetUserId(temp.get())
            << " into userdb '" << db_name << "'...";
...
bool UserDictManager::UpgradeUserDict(const std::string& dict_name) {
  UserDbComponent* legacy_db_component =
      UserDbComponent::Require("legacy_userdb");
  if (!legacy_db_component)
    return true;
  unique_ptr<UserDb> legacy_db(legacy_db_component->Create(dict_name));
  if (!legacy_db->Exists())
    return true;
  if (!legacy_db->OpenReadOnly() || !legacy_db->IsUserDb())
    return false;
...
bool UserDictManager::Synchronize(const std::string& dict_name) {
  ...
  std::string legacy_snapshot_file =
      dict_name + user_db_component_->extension() + ".snapshot";
